...
class EditablePicture extends HTMLElement {
  ...
  constructor() {
    ...
    this.TouchEvents = {
      move: ((event) => {
        this.TouchEvents.touches = [...event.touches].map(touch => ({
          x: touch.clientX,
          y: touch.clientY
        }));
        ...
        if (distance - this.TouchEvents.startDistance < 25) {
          return;
        }

        Studio.panel.openOnTab(30);
    ...
    this.addEventListener('touchstart', (event) => {
      if (window.bodySize ==='mobile' && event.touches.length == 2) {
        ...
        this.TouchEvents.startDistance = distance;

        this.addEventListener('touchmove', this.TouchEvents.move);
      }
